...
class AdaMixerDecoder_stodepth(CascadeRoIHead):
    _DEBUG = -1
    ...
    def __init__(self,
        ...
        self.Pl = Pl
        super(AdaMixerDecoder_stodepth, self).__init__(
            num_stages,
            stage_loss_weights,
    ...
    def _bbox_forward(self, stage, img_feat, query_xyzr, query_content, img_metas):
        ...
        if DEBUG:
            with torch.no_grad():
                torch.save(
                    bbox_results, 'demo/bbox_results_{}.pth'.format(AdaMixerDecoder_stodepth._DEBUG))
                AdaMixerDecoder_stodepth._DEBUG += 1
        return bbox_results
    ...
    def forward_train(self,
        ...
        skipper = torch.Tensor([-1, -1, -1, -1, -1, -1]).to(device)
        for stage in range(self.num_stages):
            if dist.get_rank() == 0:
                Pl = self.Pl[stage]
                if not np.random.binomial(1, Pl):
                    skipper[stage]= True
                else:
                    skipper[stage] = False
            else:
                skipper[stage] = -1
            # print('before', stage, device, skipper)
            dist.broadcast(skipper, src=0)
            if skipper[stage] is -1:
                raise TypeError
            # print('after', stage, device, skipper)
            if skipper[stage]:
                # print(f'skipping stage {stage} at device {device}')
                continue

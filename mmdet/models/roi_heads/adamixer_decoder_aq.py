...
class AdaMixerDecoder_aq(CascadeRoIHead):
    _DEBUG = -1
    ...
    def __init__(self,
        ...
        super(AdaMixerDecoder_aq, self).__init__(
            num_stages,
            stage_loss_weights,
    ...
    def forward_train(self,
        ...
        for stage in range(self.num_stages):
            ...
            num_queries = query_xyzr_new.size(1)
            imgs_whwh = imgs_whwh_keep.repeat(1, num_queries, 1)
            if (stage >= self.recycle[0]) and (stage < self.recycle[1]):
                query_xyzr = torch.cat((query_xyzr, query_xyzr_new), dim=1)
                query_content = torch.cat((query_content, query_content_new), dim=1)
            else:
                query_xyzr = query_xyzr_new
    ...
    def simple_test(self,
        ...
        for stage in range(self.num_stages):
            ...
            cls_score = bbox_results['cls_score']
            bboxes_list = bbox_results['detach_bboxes_list']
            query_xyzr_new = bbox_results['query_xyzr']

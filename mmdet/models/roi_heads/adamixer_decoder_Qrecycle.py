...
class AdaMixerDecoder_Qrecycle(CascadeRoIHead): # Fangyi: accumulated query with loss
    _DEBUG = -1
    ...
    def __init__(self,
        ...
        self.end_q = end_q
        super(AdaMixerDecoder_Qrecycle, self).__init__(
            num_stages,
            stage_loss_weights,
    ...
    def _bbox_forward(self, stage, img_feat, query_xyzr, query_content, img_metas):
        ...
        if DEBUG:
            with torch.no_grad():
                torch.save(
                    bbox_results, 'demo/bbox_results_{}.pth'.format(AdaMixerDecoder_Qrecycle._DEBUG))
                AdaMixerDecoder_Qrecycle._DEBUG += 1
        return bbox_results
    ...
    def simple_test(self,
        ...
        for stage in range(self.num_stages):
            ...
            #query_xyzr_keep = query_xyzr
            #query_content_keep = query_content
            ...
            #query_xyzr = (query_xyzr * alpha + query_xyzr_keep) / (1+alpha)
            #query_content = (query_content * alpha + query_content_keep) / (1+alpha)
